  ...
  return new Promise(async(resolve, reject) => {
    try {
      ...
      if (status === 200) {
        ...
        resolve({ developer: '@YudaMods', mess: 'no result found' })
      }
    } catch(err) {
      console.error(err)
    }
...
function Covid_Country(country) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (typeof _data[0] === 'undefined') return resolve({ developer: '@YudaMods', mess: 'Country not found' })
      let hasil = {
        total_cases: _data[0],
        recovered: _data[2],
        deaths: _data[1],
...
function Kompas_Global(){
  return new Promise(async(resolve, reject) => {
    try {
      ...
      if (_data.every(x => x === undefined)) return resolve({ developer: '@YudaMods', mess: 'no result found' })
      resolve(_data)
    } catch(err) {
      console.error(err)
    }
